// ignore_for_file: no_logic_in_create_state, use_build_context_synchronously, import_of_legacy_library_into_null_safe, must_be_immutable
...
class CambiarLogoEmpresa extends StatefulWidget {
  String token;
  CambiarLogoEmpresa(this.token, {Key key}) : super(key: key);

  @override
  State<CambiarLogoEmpresa> createState() => _CambiarLogoEmpresaState(token);
}

class _CambiarLogoEmpresaState extends State<CambiarLogoEmpresa> {
  String token;
  _CambiarLogoEmpresaState(this.token);
  ...
  void _subir_imagen() async {
    try {
      var request = http.MultipartRequest('POST', Uri.parse('${UrlApi().url}/api/img/empresa'));
